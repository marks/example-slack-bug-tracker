// This alternative to app.js supports OAuth installations,
//   allowing one app instance to support multiple Slack workspaces.
//   If you just need to install the app on one workspace, app.js will be simpler.
...
// Instantiate Slack SocketModeReceiver with OAuth support
const socketModeReceiverWithOAuth = new SocketModeReceiver({
  appToken: EnvVars.SLACK_APP_TOKEN,
  clientId: EnvVars.SLACK_CLIENT_ID,
  clientSecret: EnvVars.SLACK_CLIENT_SECRET,
  stateSecret: EnvVars.SLACK_STATE_SECRET,
  ...
  installationStore: {
    storeInstallation: async (installation) => {
      ...
      if (installation.isEnterpriseInstall && installation.enterprise !== undefined) {
        // TODO: add support for org-wide app installation
        throw new Error('This app does not support org-wide installations');
      }
      if (installation.team !== undefined) {
        // single workspace app installation
        // TODO upsert on team ID field
        // TODO parameterize field names

        // Look up existing record with workspace ID
        const recordWithTeamId = await airtableTable.select({
          filterByFormula: `{Slack WS ID} = "${installation.team.id}"`,
          maxRecords: 1
        }).all()

        let teamRecord
        if (recordWithTeamId.length > 0) {
          // update existing record
          teamRecord = await airtableTable.update(recordWithTeamId[0].id, {
            'Slack WS ID': installation.team.id,
            'Slack Installation JSON': JSON.stringify(installation)
          })
        } else {
          // create new record
          teamRecord = await airtableTable.create({
            'Slack WS ID': installation.team.id,
            'Slack Installation JSON': JSON.stringify(installation)
          });
        }
        ...
        return JSON.parse(teamRecord.fields['Slack Installation JSON'])
      }
    ...
    fetchInstallation: async (installQuery) => {
      ...
      if (installQuery.teamId !== undefined) {
        // single workspace app installation lookup
        // TODO parameterize field 
        const recordWithTeamId = await airtableTable.select({
          filterByFormula: `{Slack WS ID} = "${installQuery.teamId}"`,
          maxRecords: 1
        }).all()

        if(recordWithTeamId.length > 0) {
          return JSON.parse(recordWithTeamId[0].fields['Slack Installation JSON'])
        } else {
          throw new Error('No installation found for team ID ' + installQuery.teamId)
        }
      }
      throw new Error('Failed fetching installation');
    },
    // TODO understand when this gets used (https://community.slack.com/archives/CHF1FKX4J/p1644791860933559)
    deleteInstallation: async (installQuery) => {
      ...
      if (installQuery.teamId !== undefined) {
        ...
        if (recordWithTeamId.length > 0) {
          await recordWithTeamId[0].updateFields({
            'Deleted?': 'checked'
          })
          return true
        } else {
...
// Initializes your app with the receiver defined above
const app = new App({
  receiver: socketModeReceiverWithOAuth,
  logLevel: EnvVars.LOG_LEVEL || 'debug',
})
